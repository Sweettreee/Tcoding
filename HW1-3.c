...
//****** 전체 로직 *****
// 1. books에 새로운 책들 저장, cur_books에  현재 책들 저장
// 2. cur_books의 중복되는 항목 삭제
// 3. books와 cur_books를 합치고 아스키 코드 순으로 내림차순(None이 가장 위로 가고 가장 낮은 숫자가 맨 밑으로 가게됌)
// 4. 현재 부족한 책장 공간 계산 후 부족하면 Remove_Same_Rank() 실행, 아니라면은 6번으로 넘어감
// 5. 0부터 가장 높은 랭크 미만의 숫자를 탐색하면서 가장 낮은 랭크의 책들부터 삭제(삭제하는 항목과 같은 랭크의 항목들도 무조건 삭제)
// 6. 알파벳 순서로 정렬
// 7. 최종으로 부족한 책장 공간의 갯수를 계산
// 8. final_insuff_num <= 0 --> 전체 출력
//    final_insuff_num > 0 --> Your TBR is out of control Clara! 출력
